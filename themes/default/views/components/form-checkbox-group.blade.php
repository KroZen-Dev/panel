    ...
    $normalizedOptions = collect($options)->map(function ($option, $key) {
        if (is_array($option)) {
            return [
                ...
                'label' => $option['label'] ?? $option['name'] ?? $option['title'] ?? $option['value'] ?? $key,
    ...
    @else
        <div class="{{ $stacked ? 'space-y-2' : 'flex flex-wrap gap-3' }}">
            @foreach ($normalizedOptions as $option)
                <label class="inline-flex items-start gap-2 rounded-lg border border-gray-200 bg-white/80 px-3 py-2 text-sm text-gray-700 shadow-sm transition hover:border-accent-400 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-200">
                    <input type="checkbox"
