...
<x-form-group :label="$label" :for="$fieldName" :description="$description" :tooltip="$tooltip" :required="$required">
    <select @if ($inputName) name="{{ $inputName }}" @endif
        id="{{ $fieldName }}"
        ...
        @foreach ($normalizedOptions as $option)
            <option value="{{ $option['value'] }}"
                @if ($multiple && in_array($option['value'], $selectedValues ?? [], true)) selected @endif
                @if (!$multiple && ($selectedValue ?? '') === $option['value']) selected @endif
